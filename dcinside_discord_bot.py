...
async def on_message(message):
    ...
    if message.content == '!도움말':
        ...
        embed = discord.Embed(title="명령어 사용법", \
                description="모니터할 갤러리 추가하기: !추가 갤러리이름\n \
                    ...
                    try:
                        ...
                        for gall in gallery_list:
                            ...
                            if gall_dict[gall][1] == 0:
                                gall_url = 'http://gall.dcinside.com/board/lists/?id={}'.format(gall_name_Eng)
                            elif gall_dict[gall][1] == 1:
                                gall_url = 'http://gall.dcinside.com/mgallery/board/lists/?id={}'.format(gall_name_Eng)
                            if df.size == 0:
                                pass
                            else:
                                gall_name = gall + " 갤러리"
                                for i in range(df.shape[0]):
                                    embed=discord.Embed(title=gall_name, description=df.iloc[i,1])
                                    embed.add_field(name="작성자", value=df.iloc[i,2], inline=True)
                                    embed.add_field(name="시간", value=df.iloc[i,3], inline=True)
                                    embed.set_footer(text=gall_url)
                                    await message.channel.send(embed=embed) 
                                    await message.channel.send("ㅤ")
